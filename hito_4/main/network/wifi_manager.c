...
static esp_err_t scan_get_handler(httpd_req_t *req) {
  ...
  httpd_resp_sendstr_chunk(req, "<!DOCTYPE html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'><title>ESP32 WiFi</title><style>body{margin:0;font-family:Inter,Roboto,Arial;background:#000;color:#fff;text-align:center;}h1{font-weight:400;margin:20px 0;}.netlist{display:flex;flex-direction:column;align-items:center;gap:12px;padding:10px;}.card{width:90%;max-width:400px;background:#111;border:1px solid #222;border-radius:16px;padding:16px;box-shadow:0 0 8px rgba(255,255,255,0.05);text-align:left;}.ssid{font-size:18px;font-weight:500;}.rssi{font-size:14px;color:#aaa;margin-bottom:12px;}input[type=password]{width:100%;padding:8px;border:none;border-radius:8px;margin-bottom:12px;}.btn{display:block;width:100%;padding:12px;border:none;border-radius:24px;background:#fff;color:#000;font-weight:600;font-size:16px;cursor:pointer;transition:0.2s;}.btn:hover{background:#ddd;}</style></head><body><h1>Redes WiFi</h1><div class='netlist'>");
  for (int i = 0; i < ap_num; i++) {
    char ssid[33];
    memcpy(ssid, ap_records[i].ssid, sizeof(ap_records[i].ssid));
    ssid[32] = '\0';
    char chunk[512];
    snprintf(chunk, sizeof(chunk), "<div class='card'><div class='ssid'>%s</div><div class='rssi'>Señal: %d dBm</div><form method='POST' action='/config'><input type='hidden' name='ssid' value='%s'><input type='password' name='pass' placeholder='Contraseña'><button class='btn' type='submit'>Conectar</button></form></div>", ssid, ap_records[i].rssi, ssid);
...
httpd_handle_t wifi_manager_start_webserver(void) {
  ...
  httpd_uri_t conf_uri = { .uri = "/config", .method = HTTP_POST, .handler = config_post_handler, .user_ctx = NULL };
...
static void start_wifi_apsta(const char *ssid, const char *pass) {
  ...
  uint8_t mac[6];
  if (esp_wifi_get_mac(WIFI_IF_AP, mac) != ESP_OK) esp_wifi_get_mac(WIFI_IF_STA, mac);
  char ap_ssid[32];
